...
> 先讲一个餐厅的比喻：你（用户）用大白话说需求 → 服务员（LLM）把它写成一张**规范的点菜单**，菜名只能从菜单上选 → 后厨（执行器）核对菜单合法性后照单做菜。**LLM 就是服务员：它擅长听懂人话，但我们不允许它进后厨**——这是整个系统安全性的根基。

**三块积木**：

| 积木 | 是什么 | 谁来写 | 关键性质 |
|---|---|---|---|
| 界面说明书（控件树） | 界面不是画死的，而是按 JSON 说明书实时渲染：有哪些字段/按钮、谁在上谁在下。Agent 靠读它来"看见"界面 | 应用自带（随代码发布，不可变） | 改说明书 = 改界面 |
| 点菜单（布局意图 DSL） | 格式固定、动作封闭的变更清单，只有九种动作：移动/隐藏/显示/折叠/改名/分组/调列/调属性 | **LLM 的唯一输出** | 像乐谱：LLM 写谱，不同乐器（执行器）都能演奏；可预览、可撤销、可存档 |
| 执行器（引擎） | 真正动手改说明书的手；不认识 LLM，只认识点菜单 | 系统内置（纯函数） | 预览与正式应用用同一段代码 |

**一次修改的完整旅程**（以"把提交审核按钮挪到表单上方"为例，全程约 2 秒）：

1. **用户说一句话**（输入或点高频指令 chips）；
2. **Agent 翻开界面说明书**：把当前页面的控件清单（id/类型/标签/位置）连同用户的话交给 LLM——它必须引用真实存在的控件，不许编造；
3. **LLM 写出点菜单**（不改任何代码）：

   ```json
   { "action": "move", "target": "button-submit", "before": "form-ticket" }
   ```

4. **三道安检**：① 是合法 JSON 吗 → ② 动作在九种封闭词汇表里吗 → ③ 提到的控件真实存在吗。任何一道不过关就把**错误原文发回 LLM 重写**（≤2 次），再不行降级为用户点选候选控件，绝不硬改；
5. **预览**：改前/改后对比 + 变更清单，不点"应用"什么都不会发生；
6. **执行器照单修改说明书**：界面立即重绘；按钮挪了位置，点击触发的仍是原业务动作——只挪位置，不碰逻辑；
7. **存档为差量**：原始应用一个字节都没改，本次修改成为一条差量记录，可单独撤销。

**三道安全保证**：

| 保证 | 机制 |
|---|---|
| 结构上不可能越界 | LLM 只能"点菜"，菜单上没有"改数据/改金额/改审批流"这些菜——业务逻辑变更在 DSL 语法层面无法表达 |
| 错误在最低成本处暴露 | 理解偏了会卡在安检或预览环节被人看见并否决，真实生效前还有人工确认闸门 |
| 一切皆可撤销 | 原始应用零修改，定制全部是差量；每条差量可单独撤销，升级后自动重放（路线 B） |

**一套大脑，两只手**（A/B 两条路线的分叉点在执行器，理解层完全相同）：

| | 路线 A（自研 Web 应用） | 路线 B（存量 ERP，SAP 式） |
|---|---|---|
| 执行者 | SDK 执行器：改应用自己的渲染配置 | 适配层执行器：把 DSL 翻译成**厂商差量**，写入厂商适配层 |
| 差量存哪 | 应用侧"SDK 差量"（可 diff 查看、可导出变更包交开发商合入标准版） | 厂商适配机制（原型：SAP Flexibility），按**角色变体**存储 |
| 受什么约束 | 控件树 schema | **厂商适配能力边界**（只允许厂商明文支持的操作：字段显隐/改名/列序/变体） |
| 升级后 | 变更包随发版合入 | 厂商升级 → 新底座 + 差量**自动重放**，定制保留（§6.4 升级模拟） |

路线 B 的执行器**不碰应用本身**，它只是厂商适配层的"自动化前端"：传统 Adapt UI 要人工点选拖拽，我们把这一步变成说一句话。所有改动走的都是厂商明文支持的差量机制——这正是 IT 部门能接受它的原因。
